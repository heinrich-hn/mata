...
const colors = {
  primary: pdfColors.navy,
  secondary: pdfColors.charcoal,
  accent: pdfColors.success,
  warning: pdfColors.warning,
  danger: pdfColors.danger,
  lightBg: pdfColors.offWhite,
  border: [221, 221, 221] as [number, number, number],
  text: {
    primary: pdfColors.textPrimary,
    secondary: pdfColors.darkGray,
    light: pdfColors.textMuted,
  },
};
...
export function exportLoadConsignmentsToPdf(
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
      `Generated by ${SYSTEM_NAME} \u2022 ${format(new Date(), "dd MMM yyyy HH:mm")}`,
      pageWidth / 2,
      pageHeight - 5,
      { align: "center" }
    );
