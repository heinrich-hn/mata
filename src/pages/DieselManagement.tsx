    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-6">
          ...
          <Card className={pendingCostRecords.length > 0 ? 'border-amber-500' : ''}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Pending Cost</CardTitle>
              <DollarSign className="h-4 w-4 text-amber-500" />
            </CardHeader>
            <CardContent>
              <div className={`text-2xl font-semibold ${pendingCostRecords.length > 0 ? 'text-amber-600' : ''}`}>
                {pendingCostRecords.length}
              </div>
              {pendingCostRecords.length > 0 && (
                <p className="text-xs text-amber-600 mt-1">
                  Needs cost per litre
                </p>
              )}
            </CardContent>
          </Card>
