...
const TripExpensesSection = ({ trips, onViewTrip }: TripExpensesSectionProps) => {
  ...
  const filteredExpenses = useMemo(() => {
    return expenses.filter((expense) => {
      ...
      if (filterStatus !== 'all') {
        ...
        // System-generated costs (diesel linkage, route expenses, system rates) never need attention
        const isAutoGenerated = expense.is_system_generated === true;
        // Once approved/resolved or auto-generated, the cost no longer needs attention
        const needsAttention = (isResolved || isAutoGenerated) ? false : (hasUnresolvedFlag || isMissingSlip);
  ...
  const stats = useMemo(() => {
    ...
    expenses.forEach((expense) => {
      ...
      // System-generated costs are pre-verified — skip from attention/flag counts only
      if (expense.is_system_generated) {
        if (expense.investigation_status === 'resolved') resolvedCount++;
        return;
      }
