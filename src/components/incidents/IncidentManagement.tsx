...
const useDownloadIncidentPdf = () => {
  ...
  const download = useCallback(async (incident: Incident) => {
    ...
    try {
      ...
      const [{ data: documents = [] }, { data: rawTimeline = [] }, { data: carReport }] = await Promise.all([
        supabase
      ...
      await generateIncidentPDF({ incident, documents, timeline, carReport });
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="start">
                                  ...
                                  <DropdownMenuItem onClick={() => openDialog("createCAR", incident)}>
                                    <FileText className="mr-2 h-4 w-4" /> CAR Report
                                  </DropdownMenuItem>
                                  <DropdownMenuSeparator />
