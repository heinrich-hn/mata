...
/**
 * Build the inspection PDF document (without saving). Useful when the caller
 * needs the raw `jsPDF` instance to convert to a Blob for sharing/emailing.
 */
...
/**
 * Generate a PDF report for a vehicle inspection and trigger a download.
 */
export async function generateInspectionPDF(
  inspection: InspectionData,
  items?: InspectionItem[]
): Promise<void> {
  const { doc, fileName } = buildInspectionPDF(inspection, items);
  doc.save(fileName);
}
