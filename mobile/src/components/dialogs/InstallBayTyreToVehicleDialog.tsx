    ...
    const handleInstallation = async () => {
        ...
        try {
            ...
            toast({
                ...
                description: `${tyre.brand} ${tyre.model} installed on ${vehicle.registration_number} at ${formData.position}`,
            });
                    ...
                    <CardContent className="pt-3 pb-2">
                        <div className="grid grid-cols-2 gap-2 text-sm">
                            <div>
                                ...
                                <p className="font-semibold text-sm">{tyre.brand} {tyre.model}</p>
                            </div>
                            <div>
                                <span className="text-muted-foreground text-xs">Size</span>
                                <p className="font-semibold text-sm">{tyre.size}</p>
                            </div>
                            <div>
                                <span className="text-muted-foreground text-xs">Tread</span>
                                <p className="font-semibold text-sm">{tyre.current_tread_depth ? `${tyre.current_tread_depth}mm` : '-'}</p>
                            </div>
                            <div>
                                <span className="text-muted-foreground text-xs">Lifetime KM</span>
                                <p className="font-semibold text-sm">{(tyre.km_travelled || 0).toLocaleString()}</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <Card className="bg-green-50 dark:bg-green-950/20 border-green-200">
                        <CardContent className="pt-3 pb-2">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="font-semibold text-sm text-green-800 dark:text-green-200">Confirm Installation</span>
                            </div>
                            <div className="space-y-1.5 text-sm">
                                <p><span className="text-muted-foreground">Tyre:</span> {tyre.brand} {tyre.model}</p>
                                <p><span className="text-muted-foreground">Vehicle:</span> {selectedVehicle?.fleet_number} - {selectedVehicle?.registration_number}</p>
                                <p><span className="text-muted-foreground">Position:</span> {formData.position} {positionLabel ? `(${positionLabel})` : ''}</p>
                                <p><span className="text-muted-foreground">Odometer:</span> {parseFloat(formData.installationReading).toLocaleString()} km</p>
                ...
                <DialogFooter>
                    {!showConfirmation ? (
                        ...
                        <div className="flex gap-2 w-full">
                            ...
                            <Button className="flex-1" onClick={handleInstallation} disabled={loading}>
