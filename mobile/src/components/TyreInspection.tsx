...
const TyreInspection = () => {
  ...
  const categoryConfig: Record<string, { label: string; tyreCount: string }> = {
    Horse: { label: "Horses", tyreCount: "10+SP tyres" },
    Reefer: { label: "Reefers", tyreCount: "12 tyres" },
    Trailer: { label: "Trailers", tyreCount: "12 tyres" },
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {sortedCategories.length === 0 ? (
          <p className="text-muted-foreground text-sm px-1">No vehicles found</p>
        ) : (
              ...
              >
                <CollapsibleTrigger asChild>
                  <Button
                    variant="ghost"
                    className={cn(
                      "w-full justify-between px-3 py-2.5 h-auto rounded-xl border bg-gradient-to-r transition-all duration-200",
                      categoryColors[category] || categoryColors.Other
                    )}
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <ChevronRight className={cn("w-4 h-4 flex-shrink-0 transition-transform duration-200", isExpanded && "rotate-90")} />
                      <span className="font-bold text-sm truncate">{categoryData?.label || category}</span>
                    </div>
                    <span className="px-2 py-0.5 rounded-full bg-primary/10 text-primary text-xs font-semibold flex-shrink-0">
                      {totalVehicles}
                    </span>
                  </Button>
                </CollapsibleTrigger>
                        ...
                        return (
                          <div key={vehicle.id}>
                            <Button
                              ...
                              size="sm"
                              className={cn(
                                "w-full h-auto py-2 px-3 rounded-lg transition-all duration-150",
                                isSelected ? "shadow-md" : "active:bg-muted/80"
                              )}
                              onClick={() => setVehicleId(isSelected ? "" : vehicle.id)}
                            >
                              <div className="flex items-center justify-between w-full gap-2">
                                <div className="flex items-center gap-2 min-w-0">
                                  <ChevronRight className={cn("w-3 h-3 flex-shrink-0 transition-transform duration-200", isSelected && "rotate-90")} />
                                  <span className="font-bold text-sm">{fleetNumber}</span>
                                  <span className="text-xs text-muted-foreground truncate">{vehicle.registration_number}</span>
                                </div>
                                {fleetConf && (
                                  <span className={cn(
